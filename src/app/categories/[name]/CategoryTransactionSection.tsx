...
import { ChartGranularityPills } from "./ChartGranularityPills.tsx";
import { useExcludedFilterSync } from "./useExcludedFilterSync.ts";
...
// Locked column widths so paging doesn't reflow the table.
...
export function CategoryTransactionSection({
  transactions,
  accounts,
  chartData,
  granularity,
  ...
  // Bar selection is pure client state — clicking a bar isolates the table to
  // that bucket instantly, with no server round-trip. Only highlight a key that
  // exists in the current chart (a stale key highlights nothing, not all).
  ...
  // The server re-renders with a fresh window set on filter/granularity changes;
  // reset the page AND drop any bar selection (its key may no longer apply).
  ...
  // paginate — so sorting on (e.g.) amount orders every row, not just the page.
